...
6. **文档管理** — "是否需要管理文档？1.是 2.否。如果是，文档类型：1.PRD 2.设计文档 3.测试用例 4.会议纪要 5.其他"
7. **版本追踪**（可跳过） — "文档需要版本追踪吗？1.是 2.否"
8. **记忆需求**（可跳过） — "需要长期记忆吗？1.是 2.否。如果是，记住什么类型的信息？"
9. **特殊要求**（可跳过） — "还有其他规则或约束吗？比如PRD必须包含某些字段。（可回复'跳过'）"
10. **确认** — 汇总所有配置为简表，让用户确认或修改。
