...
export function buildMcpPromptSection(manifest: McpToolManifest): string {
  ...
  return [
    ...
    "The runner prepares repository context automatically. Never call project.ensure or clone/locate a repository yourself.",
    "project.publish is the only project write-to-remote operation. Call it only after explicit user authorization to submit or Push. Choose a concise meaningful bot/<task-name> branch; the service validates it and the current commit.",
